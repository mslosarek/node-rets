...
async function ParseRetsResponseXML(xmlContent) {
  ...
  if (delimiterString) { // process compact format
    try {
      const delimiter = String.fromCharCode(parseInt(delimiterString, 16));
      jsonContent.RETS.COLUMNS = jsonContent.RETS.COLUMNS.split(delimiter);
      ...
      .map(dataArray => {
        const dataObject = object(jsonContent.RETS.COLUMNS, dataArray);
        delete dataObject[''];
        return dataObject;
      });
    } catch (err) {
  ...
  } else if (retsResponse) { // process rets-response
